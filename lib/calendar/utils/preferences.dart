...
import 'package:mailandcalendaruppa/calendar/param/pref_key.dart';
...
import 'package:mailandcalendaruppa/calendar/models/preferences/prefs_theme.dart';
...
class PreferencesProviderState extends State<PreferencesProvider> {
  ...
  ///mail client imap host server address
  String _imapHostServer;

  ///mail client imap server port
  int _imapPort = 993;
  ...
  String get username =>_username ?? PrefKey.defaultUsername;

  setUsername(String newUsername, [state = false]){
    if(username == newUsername) return;

    _updatePref(()=> _username=newUsername, state);
    widget.prefs.setString(PrefKey.username, _username);
  }

  String get password =>_password ?? PrefKey.defaultPassword;

  setPassword(String newPassword, [state = false]){
    if(password == newPassword) return;

    _updatePref(()=> _username=newPassword, state);
    widget.prefs.setString(PrefKey.password, _password);
  }

  String get imapHostServer =>_imapHostServer ?? PrefKey.defaultImapHostServer;

  setImapHostServer(String newImapHostServer, [state = false]){
    if(imapHostServer == newImapHostServer) return;

    _updatePref(()=> _imapHostServer=newImapHostServer, state);
    widget.prefs.setString(PrefKey.imapHostServer, _imapHostServer);
  }

  int get imapPort =>_imapPort ?? PrefKey.defaultImapPort;

  setImapPort(int newImapPort, [state = false]){
    if(imapPort == newImapPort) return;

    _updatePref(()=> _imapPort= newImapPort, state);
    widget.prefs.setInt(PrefKey.imapPort, _imapPort);
  }
